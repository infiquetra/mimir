...
void main() {
  ...
  final incomingDamagePattern = RegExp(
    r'^(\d+) from (.*?)\s+-\s+(.*?)\s+-\s+(.*)$',
  );
  final outgoingDamagePattern = RegExp(
    r'^(\d+) to (.*?)\s+-\s+(.*?)\s+-\s+(.*)$',
  );
  final incomingMissPattern = RegExp(
    r'^(.*?) misses you completely\s+-\s+(.*)$',
  );
  ...
  for (var line in lines) {
    ...
    if (match != null) {
      ...
      if (inDam != null) {
        print(
          '  => INCOMING DAMAGE: ${inDam.group(1)} from ${inDam.group(2)} (${inDam.group(3)})',
        );
      }
      ...
      if (outDam != null) {
        print(
          '  => OUTGOING DAMAGE: ${outDam.group(1)} to ${outDam.group(2)} (${outDam.group(3)})',
        );
      }
