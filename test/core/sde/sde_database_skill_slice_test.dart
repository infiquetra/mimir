...
void main() {
    ...
    await database.batch((batch) {
      batch.insertAll(database.sdeCategories, [
        SdeCategoriesCompanion.insert(
          categoryId: const drift.Value(16),
          categoryName: 'Skill',
        ),
        SdeCategoriesCompanion.insert(
          categoryId: const drift.Value(6),
          categoryName: 'Ship',
        ),
      ]);
  ...
  group('deleteSkillSlice', () {
    test('removes skill types and their prerequisites', () async {
      ...
      final remainingRequirements = await database.getSkillPrerequisites(3301);
      expect(remainingRequirements, isEmpty);
    });

    test(
      'leaves dogma attributes, effects and industry data untouched',
      () async {
        await database.deleteSkillSlice(
          skillGroupIds: [255],
          skillTypeIds: [3301],
